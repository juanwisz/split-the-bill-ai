        ...
        tools: {
          ...
          createDocument: {
            ...
            execute: async ({ title, kind }) => {
              ...
              if (session.user?.id) {
                await saveDocument({ id, title, kind, content: draftText, userId: session.user.id });
              }
          ...
          updateDocument: {
            ...
            execute: async ({ id, description }) => {
              ...
              if (session.user?.id) {
                await saveDocument({ id, title: document.title, content: draftText, kind: document.kind, userId: session.user.id });
              }
        
              return { id, title: document.title, kind: document.kind, content: 'The document has been updated successfully.' };
            },
          },
          requestSuggestions: {
            ...
            execute: async ({ documentId }) => {
              ...
              return { id: documentId, title: document.title, kind: document.kind, message: 'Suggestions have been added to the document' };
            },
          },
          addTransaction: {
          ...
          calculateBalance: {
            ...
            execute: async () => {
              try {
                ...
                // Format only the settlement instructions
                ...
                return {
                  id,
                  type: 'balance',
                  content: `Optimal settlement plan:\n${paymentInstructions}`,
                  data: { 
                    simplifiedTransactions,  // Only pass what we need
                    balances: {} // Empty object since we won't display it
                  }
                };
              } catch (error) {
                ...
                return {
                  id,
                  type: 'balance',
                  content: 'Failed to calculate balances',
                  error: true
                };
              }
            }
